...
export default function EditarProduto({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setError("Erro ao carregar produto: " + (error as Error).message);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Falha ao atualizar produto: " + (error as Error).message);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (!profile) {
    return <div className="container py-12">Carregando perfil...</div>;
  }
    ...
    return (
      <div className="container py-24 max-w-4xl mx-auto">
        <PageHeader
          ...
          text="Atualize os dados do produto"
        />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="stripe_product_id">
                  ID do Produto no Stripe (opcional)
                </Label>
                <Input
